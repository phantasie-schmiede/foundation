...
/**
 * Class FlexFormMarkerServiceTest
 *
 * @package PSBits\Foundation\Service\Configuration
 */
class FlexFormMarkerServiceTest extends UnitTestCase
{
    public const string TEST_STRING_CONSTANT  = 'test_constant_value';
    public const int    TEST_INT_CONSTANT     = 42;

    private FlexFormMarkerService $subject;

    protected function setUp(): void
    {
        parent::setUp();
        $this->subject = new FlexFormMarkerService();
    }
    ...
    public static function processDataProvider(): Generator
    {
        ...
        yield 'string class constant marker is replaced with constant value' => [
            '###\PSBits\Foundation\Service\Configuration\FlexFormMarkerServiceTest::TEST_STRING_CONSTANT###',
            self::TEST_STRING_CONSTANT,
        ];

        yield 'int class constant marker is replaced with string representation of constant value' => [
            '###\PSBits\Foundation\Service\Configuration\FlexFormMarkerServiceTest::TEST_INT_CONSTANT###',
        ...
        yield 'multiple markers in XML are all replaced' => [
            '<numIndex index="0">###\PSBits\Foundation\Tests\Examples\BackedEnum::Delta###</numIndex>'
            . '<numIndex index="1">###\PSBits\Foundation\Tests\Examples\BackedEnum::Epsilon###</numIndex>',
            '<numIndex index="0">' . BackedEnum::Delta->value . '</numIndex>'
            . '<numIndex index="1">' . BackedEnum::Epsilon->value . '</numIndex>',
        ];
    ...
    {
        self::assertEquals($expectedResult, $this->subject->process($xml));
    }
}
